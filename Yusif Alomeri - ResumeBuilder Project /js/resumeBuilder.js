...
var bio = {
        "name": "Yusif Alomeri",
        "role": "Web Designer",
        "welcomeMsg" : "Never judge a fish by its ability to climb a tree",
...
function displayWork(){
    $("#workExperience").append(HTMLworkStart);
        work.job.forEach(function(job,index){
            console.log(job);
                var formattedEmployer = HTMLworkEmployer.replace("%data%", work.job[index].employer);
                var formattedTitle = HTMLworkTitle.replace("%data%", work.job[index].title);
                var fromattedEmployerTitle = formattedEmployer + formattedTitle;
                $(".work-entry:last").append(fromattedEmployerTitle);
    
                var formattedLocation = HTMLworkLocation.replace("%data%", work.job[index].location);
                $(".work-entry:last").append(formattedLocation);
    
                var formattedDates = HTMLworkDates.replace("%data%", work.job[index].dates);
                $(".work-entry:last").append(formattedDates);
    
                var formattedDescription = HTMLworkDescription.replace("%data%", work.job[index].description);
...
function displayEducation(){
    ...
    education.school.forEach(function(school,index) { 
            console.log(school);
            var formattedSchoolName = HTMLschoolName.replace("%data%",education.school[index].name);
            $(".education-entry").append(formattedSchoolName);

            var formattedSchoolDegree = HTMLschoolDegree.replace("%data%", education.school[index].degree);
            $(".education-entry").append(formattedSchoolDegree);

            var formattedSchoolDates = HTMLschoolDates.replace("%data%", education.school[index].dates);
            $(".education-entry").append(formattedSchoolDates);

            var formattedSchoolLocation = HTMLschoolLocation.replace("%data%", education.school[index].location);
            $(".education-entry").append(formattedSchoolLocation);

            var formattedSchoolMajor = HTMLschoolMajor.replace("%data%", education.school[index].major);
